...
class RuffnoteStrategy extends OpauthStrategy {
	...
	public function request() {
		$url = 'https://ruffnote.com/login/oauth/authorize';
		$params = array(
	...
	public function oauth2callback() {
		if (array_key_exists('code', $_GET) && !empty($_GET['code'])) {
			$code = $_GET['code'];
			$url = 'https://ruffnote.com/login/oauth/access_token';
	...
	private function user($access_token) {
		$user = $this->serverGet('https://api.ruffnote.com/user', array('access_token' => $access_token), null, $headers);
