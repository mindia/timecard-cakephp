		...
		<tbody>
			...
			<tr>
				...
				<td>
					...
					<span class="my-counter"><?php while($s>=3600){$s-=3600;$h++;}while($s>=60){$s-=60;$m++;}echo"$h hour $m min $s sec" ?></span>
					<?php else:?>
					<span><?php while($s>=3600){$s-=3600;$h++;}while($s>=60){$s-=60;$m++;}echo"$h hour $m min $s sec" ?></span>
					<?php endif?>
				</td>
